...
describe("OptionInputSchema", () => {
	describe("Valid inputs", () => {
		...
		test("number at boundaries (1 and 9)", () => {
			expect(
				OptionInputSchema.safeParse({ number: 1, label: "A" }).success,
			).toBe(true);
			expect(
				OptionInputSchema.safeParse({ number: 9, label: "B" }).success,
			).toBe(true);
		});
	});
	...
	describe("Valid inputs", () => {
		...
		test("defaults are applied", () => {
			...
			if (result.success) {
				...
				expect(result.data.voteType).toBe("NUMBER");
			}
		});
	});
...
describe("UpdatePollSchema", () => {
	describe("Valid inputs", () => {
		...
		test("all status enum values", () => {
			expect(UpdatePollSchema.safeParse({ status: "DRAFT" }).success).toBe(
				true,
			);
			expect(UpdatePollSchema.safeParse({ status: "LIVE" }).success).toBe(true);
			expect(UpdatePollSchema.safeParse({ status: "ENDED" }).success).toBe(
				true,
			);
		});
