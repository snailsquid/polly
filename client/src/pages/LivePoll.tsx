...
export default function LivePoll() {
	...
	const { votes: wsVotes, pollStatus } = useWebSocket(id, (updatedPoll) => {
	...
	// Initialize the live theme from the poll's configured liveTheme (once).
	useEffect(() => {
		if (poll && !themeInitRef.current) {
			themeInitRef.current = true;
			setTheme(poll.liveTheme);
		}
	}, [poll]);
	...
	useEffect(() => {
		...
		const calculate = () => {
			const elapsed =
				(Date.now() - new Date(liveRun.createdAt).getTime()) / 1000;
			const remaining = Math.max(
				0,
				Math.ceil((liveRun!.duration ?? 0) - elapsed),
			);
		...
		<div className="min-h-screen flex flex-col">
			<header className="border-b p-4 flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					...
					{isOwner && isLive && (
						<Button
							onClick={() => endMutation.mutate(poll.id)}
